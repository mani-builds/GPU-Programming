...
@triton.jit
def fused_softmax_kernel(x_ptr, output_ptr,input_row_stride, output_row_stride, n_col, BLOCK_SIZE:tl.constexpr):
    # map data to programs
    pid = tl.program_id(0) # equal to number of rows
    col_offsets =  tl.arange(0, BLOCK_SIZE) # block_size = col width
    mask = tl.arange(0, BLOCK_SIZE) < n_col

    row_ptr_start = x_ptr + pid * input_row_stride
    # load data
    ...
    output_ptrs = output_ptr + pid * output_row_stride
...
def softmax_kernel(x: torch.Tensor):
    ...
    fused_softmax_kernel[grid](x, output, input_row_stride=x.stride(0),
                               output_row_stride=output.stride(0), n_col=n_col, BLOCK_SIZE=BLOCK_SIZE)
...
assert torch.allclose(output_triton, output_torch, atol=1e-7, rtol=1e-5)
